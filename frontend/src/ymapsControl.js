...
function createBalloonContentLayout() {
  balloonContentLayout = ymaps.templateLayoutFactory.createClass(
    `<div>
        <p id="max"> Max: {{ properties.areaStatistics.info_heatmap_max | default:"No data"  }} </p>
        <p id="min"> Min: {{ properties.areaStatistics.info_heatmap_min | default:"No data"  }} </p>
        <p id="average">Average: {{ properties.areaStatistics.info_heatmap_avg | default:"No data" }} </p>
        <button id="delete-heatmap"> Удалить </button>
      </div>`,
    {
      build: function () {
        balloonContentLayout.superclass.build.call(this);
        document.querySelector("#delete-heatmap").onclick =
          this.delete.bind(this);
      },

      clear: function () {
        document
          .querySelector("#delete-heatmap")
...
const areaTypeStream$ = new Subject().pipe(distinctUntilChanged());
...
function existSameArea(areaInfoWithType) {
  const isSame = (geoObj, comparedAreaInfo) => {
    ...
    if (comparedAreaInfo.coords.length === gCoords?.length) {
      return (
        JSON.stringify(comparedAreaInfo.coords) === JSON.stringify(gCoords)
      );
    }
    return false;
  };
