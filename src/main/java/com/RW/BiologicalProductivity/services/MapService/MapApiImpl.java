...
public class MapApiImpl implements MapAPI {
    ...
    public byte[] getSectorAsBytes(double[][] twoPoints, TypeMap typeMap) throws IOException {
        try {
            ...
            System.err.println("MapAPI: Region with map '" + typeMap +"' not detected");
            return new byte[0];
        }
    }
    public byte[] getSectorAsBytes(double[][] twoPoints, double[][] areaWordCoords, TypeMap typeMap) throws IOException {
        try {
            String region_name = region.getName();
            MapData mapData = new MapData(regionService.getMapInfo(region_name,typeMap));
